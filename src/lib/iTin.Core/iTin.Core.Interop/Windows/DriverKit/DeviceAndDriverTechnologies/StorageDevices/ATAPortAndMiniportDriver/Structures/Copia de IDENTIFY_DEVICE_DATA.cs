...
namespace iTin.Core.Interop.Windows.DriverKit.DeviceAndDriverTechnologies.StorageDevices.AtaPortAndMiniportDriver
{
    ...
    public struct IDENTIFY_DEVICE_DATA
    {
        ...
        {
            _SupportedFeatures[(int)StateFeature.NativeCommandQueuing]         = Logical.CheckBit(word076, Bits.Bit08);
            _SupportedFeatures[(int)StateFeature.PhyEventCounters]             = Logical.CheckBit(word076, Bits.Bit10);

            _SupportedFeatures[(int)StateFeature.DmaSetupAutoActivate]         = Logical.CheckBit(word078, Bits.Bit02);
            _SupportedFeatures[(int)StateFeature.InOrderDataDelivery]          = Logical.CheckBit(word078, Bits.Bit04);
            _SupportedFeatures[(int)StateFeature.SoftwareSettingsPreservation] = Logical.CheckBit(word078, Bits.Bit06);

            _SupportedFeatures[(int)StateFeature.Smart]                        = Logical.CheckBit(word082, Bits.Bit00);
            _SupportedFeatures[(int)StateFeature.SecurityMode]                 = Logical.CheckBit(word082, Bits.Bit01); 
            _SupportedFeatures[(int)StateFeature.WriteCache]                   = Logical.CheckBit(word082, Bits.Bit05);
            _SupportedFeatures[(int)StateFeature.ReadLookAhead]                = Logical.CheckBit(word082, Bits.Bit06);
            _SupportedFeatures[(int)StateFeature.ReleaseInterrupt]             = Logical.CheckBit(word082, Bits.Bit07);

            _SupportedFeatures[(int)StateFeature.AdvancedPowerManagement]      = Logical.CheckBit(word083, Bits.Bit03);
            _SupportedFeatures[(int)StateFeature.PowerUpInStandby]             = Logical.CheckBit(word083, Bits.Bit05);
            _SupportedFeatures[(int)StateFeature.AutomaticAcousticManagement]  = Logical.CheckBit(word083, Bits.Bit09);
            _SupportedFeatures[(int)StateFeature.Bit48Lba]                     = Logical.CheckBit(word083, Bits.Bit10);
            _SupportedFeatures[(int)StateFeature.DeviceConfigurationOverlay]   = Logical.CheckBit(word083, Bits.Bit11);
            
            _SupportedFeatures[(int)StateFeature.SmartErrorLogging]            = Logical.CheckBit(word084, Bits.Bit00);
            _SupportedFeatures[(int)StateFeature.SmartSelfTest]                = Logical.CheckBit(word084, Bits.Bit01);
            _SupportedFeatures[(int)StateFeature.Streaming]                    = Logical.CheckBit(word084, Bits.Bit04);

            _SupportedFeatures[(int)StateFeature.PowerManagement]              = Logical.CheckBit(word085, Bits.Bit03);
            _SupportedFeatures[(int)StateFeature.HostProtectedArea]            = Logical.CheckBit(word085, Bits.Bit10);

            _SupportedFeatures[(int)StateFeature.GeneralPurposeLogging]        = Logical.CheckBit(word087, Bits.Bit05);
